({
    ...
    handleParentActiveTab: function(component, event, helper) {
        try {
            ...
            tab = tab.get('v.id');
            if (tab == 'PayByACH') {
                if (component.get('v.paymentType').includes('Preauth')) {
                    component.set('v.paymentType', 'Deposit');
                }
    ...
    onChangeAmount: function(component, evt, helper) {
        try {
            ...
            if (obj == 'Order') {
                ...
                if (payType == 'Deposit' || payType == 'Sale') {
                    ...
                    if (amount > OrderAmount) {
                        ...
                        helper.showToastMsg('Amount should not be greater than order amount', 'Warning');
                    }
                    if (amount < OrderBalance) {
                        amount = OrderBalance;
                        component.set('v.Amount', OrderBalance);
            ...
            if (obj == 'Quote') {
                var QuoteAmount = component.get('v.quoteDetail.GrandTotal');
                var EbizAmount = component.get('v.quoteDetail.Ebiz_C__EBizCharge_Amount__c');
                var QuoteBalance = component.get('v.quoteDetail.Ebiz_C__EBizCharge_Quote_Balance__c');
                var payType = component.get('v.paymentType');
                if (payType == 'Deposit' || payType == 'Sale') {
                    if (amount > QuoteBalance) {
                        amount = QuoteBalance;
                        component.set('v.Amount', QuoteBalance);
                        helper.showToastMsg('Amount should be equal or less than quote remaining amount', 'Warning');
                    }
                } else if (payType == 'Preauth') {
                    if (amount > QuoteAmount) {
                        amount = QuoteAmount;
                        component.set('v.Amount', QuoteAmount);
                        helper.showToastMsg('Amount should not be greater than quote amount', 'Warning');
                    }
                    if (amount < QuoteBalance) {
                        amount = QuoteBalance;
                        component.set('v.Amount', QuoteBalance);
                        helper.showToastMsg('Amount should be equal or greater than quote remaining amount', 'Warning');
                    }
                }
            }
