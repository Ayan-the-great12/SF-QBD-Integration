({
    ...
    getRowActions: function(cmp, row, doneCallback) {
        ...
        if (row.Result == 'Error') {
            voidAction.disabled = 'true';
        }
        else if (row.Result == 'Declined') {
            voidAction.disabled = 'true';
        }
        else if (row.TransType == 'Voided Sale') {
            voidAction.disabled = 'true';
        }
        else if (row.Status == 'Pending' && row.TransType != 'Credit') {} else {
            voidAction.disabled = 'true';
        }
        actions.push(voidAction);
        var Refund = {
            'label': 'Refund Transaction',
            'iconName': 'utility:retweet',
            'name': 'RefundTransaction'
        };
        if (row.Status == 'Settled' && row.TransType != 'Credit') {} else {
